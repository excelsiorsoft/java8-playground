package com.excelsiorsoft.radioactive;
...
public interface MaybeAccessor<T, V> extends Accessor<T, Optional<V>> {
    static<T, V> com.excelsiorsoft.radioactive.MaybeAccessor<T, V> wrapping(Function<T, Optional<V>> getter, BiConsumer<T, Optional<V>> setter) {
        return new com.excelsiorsoft.radioactive.MaybeAccessor<T, V>() {
    ...
    default <V2> com.excelsiorsoft.radioactive.MaybeAccessor<T, V2> map(Accessor<V, V2> next, Supplier<V> missingValueSupplier, V2 defaultValue) {
        return wrapping(
                t -> get(t).map(next),
                (t, v2) -> next.set(ensuringPresent(missingValueSupplier).get(t), v2.orElse(defaultValue)));
    }

    default <V2> com.excelsiorsoft.radioactive.MaybeAccessor<T, V2> flatMap(Accessor<V, Optional<V2>> next, Supplier<V> missingValueSupplier) {
        return wrapping(
