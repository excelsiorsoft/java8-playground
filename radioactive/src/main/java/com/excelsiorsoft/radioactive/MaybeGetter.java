...
public interface MaybeGetter<T, V> extends Getter<T, Optional<V>> {
    ...
    default <V2> com.excelsiorsoft.radioactive.MaybeGetter<T, V2> map(Function<V, V2> next) {
        return t -> apply(t).map(next);
    }

    default <V2> com.excelsiorsoft.radioactive.MaybeGetter<T, V2> flatMap(Function<V, Optional<V2>> next) {
